...
def get_satellite_img(lat, lon, datetime):
    ...
    visual.rio.to_raster("App_pystac/templates/Images/current.tif", driver="COG")
    # Loading the saved .tif pacth image
    data = rasterio.open('App_pystac/templates/Images/current.tif')
    ...
    # Image is downloaded 
    with rasterio.open("App_pystac/templates/Images/current.tif") as src:
    ...
    with rasterio.open("App_pystac/templates/Images/RGB_masked2.tif", "w", **out_meta) as dest:
